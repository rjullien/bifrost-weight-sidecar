...
3. Calcul local des poids (burn-to-100 % monthly, bloqueurs weekly/rolling bruts) — **sans**
...
**Politique : « cramer le monthly à 100 % avant reset, garde-fous, secours fail-open »** —
le quota mensuel non consommé avant le reset anniversaire est **perdu**. L'algo
pousse le trafic vers les clés qui **n'atteindront pas 100 %** à leur rythme
actuel (`MonthlyDryDays == 0`), et normalise les poids actifs pour qu'ils
somment à **100** :
...
| 5a | ≥ 1 **sous-brûleur** (`DryDays == 0`, monthly restant > 0) | **tout** le poids sur ces clés (1 → `100` ; plusieurs → split par urgence puis normalize Σ=`100`) ; les clés *on track* (`DryDays > 0`) → `0` |
| 5b | 0 sous-brûleur | urgence parmi les non-bloquées avec restant > 0, normalize Σ=`100` ; sinon tout à `0` |
...
- `DryDays > 0` — projeté d'atteindre le plafond avant le reset → n'a **pas**
  besoin de trafic prioritaire pour finir de cramer.
- `DryDays == 0` — ne touchera **pas** 100 % au rythme actuel → reçoit le
  trafic (winner-take-all / split).

**Urgence** : `monthly restant (%) ÷ jours restants`. Sert uniquement à
répartir *entre* sous-brûleurs (ou en fallback 5b). Les poids appliqués sont
des **pourcentages** (Σ = 100), plus les anciennes magnitudes brutes type
`1.939` / `5.697`.
...
**Exemple (prod-like)** : N ~92 % / ~1,4 j restants (sous-brûleur ~8 %) ;
Main ~97 % / ~1,5 j (~3 %, aussi sous-brûleur si `DryDays == 0`) ; R/A
confortables on track. → poids uniquement sur N (+ Main si les deux
sous-brûlent), split par urgence, Σ = 100 ; R/A à `0`.
